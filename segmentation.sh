    ...
    handleMapscreation() {
        if [ $EXIST -eq 1 ]; then
          ...
          mrcalc CLmap\(PRE\).nii.gz 100 -mult CLmap\(PRE\)_temp.nii.gz -force
          mrgrid CLmap\(PRE\)_temp.nii.gz regrid -template ../T1_upsampled.nii.gz -datatype uint8 -interp linear CLmap_up.nii.gz -force          
                
          rm FAmap_Red\(PRE\).nii.gz FAmap_Green\(PRE\).nii.gz FAmap_Blue\(PRE\).nii.gz CLmap\(PRE\)_temp.nii.gz
    ...
    handleLesion() {
        if [ $EXIST -eq 1 ]; then
		  ...
		  mrgrid ../Segmentation/T2_raw_coreg.nii.gz regrid -voxel 0.3 -stride -1,-2,3 T2_raw_coreg_up.nii.gz -force
		  mrgrid ../Segmentation/T2_raw_24_coreg.nii.gz regrid -template T2_raw_coreg_up.nii.gz -stride -1,-2,3 T2_raw_24_coreg_resampled.nii.gz -force

		  mrcalc "$HISTO_DIR/seg_left.nii.gz" 314 -eq ROI_rostral_lh.mif -datatype bit -force
		  mrgrid ROI_rostral_lh.mif regrid -template T2_raw_coreg_up.nii.gz -datatype uint8 -stride -1,-2,3  -oversample 1,1,1 ROI_rostral_lh_T2.nii.gz -force
		  mrcalc "$HISTO_DIR/seg_right.nii.gz" 314 -eq ROI_rostral_rh.mif -datatype bit -force
		  mrgrid ROI_rostral_rh.mif regrid -template T2_raw_coreg_up.nii.gz -datatype uint8 -stride -1,-2,3 -oversample 1,1,1 ROI_rostral_rh_T2.nii.gz -force
		  ...
		  mrgrid 5tt_coreg_csf_bool.mif regrid -template T2_raw_coreg_up.nii.gz -stride -1,-2,3 -datatype uint8 -oversample 1,1,1 5tt_coreg_csf_resampled.nii.gz -force
