...
def parse_julich_brain_xml(xml_file, output_lut):
    ...
    with open(output_lut, "w") as lut_file:
        ...
        for structure in root.find("Structures"):
            num = int(structure.get("grayvalue")) + 207  # Número único da região
            ...
            lut_file.write(f"{num} {name.replace(' ', '_')}_R {r} {g} {b} 255\n")

    print(f"LUT criado com sucesso: {output_lut}")

# Exemplo de uso
xml_file = "JulichIndex_rh.xml"  # Substitua pelo caminho para o seu arquivo XML
output_lut = "JulichLUT_mrtrix_rh.txt"  # Nome do arquivo LUT de saída
